...
export function abonentInit({setAbonent, setProvider, pfilter}) {
  const {cat: {abonents}, adapters: {pouch}, job_prm, wsql} = $p;
  ...
  if(!abonent) {
    abonent = parseInt(job_prm.zone, 10);
  }
  setAbonent([abonent ? abonents.by_id(abonent) : abonents.get(), abonents.find_rows({})]);

  let provider = pouch.props._auth_provider || wsql.get_user_param('auth_provider');
