...
export function buildTree({mgr, owner, meta, selected}) {
  const {utils} = $p;
  const tree = new BaseItem(mgr.get(), null);
  tree.toggled = true;

  // получаем доступные элементы
  const selection = mgr.get_search_selector({
    _obj: owner?._obj,
    _meta: meta,
    _fld: owner?.props?._fld,
    source_mode: 'ram',
  });
  delete selection._skip;
  delete selection._top;
  const _selection = utils._selection.bind(mgr);
  const elmnts = [];
  for (const o of mgr) {
  ...
  for(const elm of elmnts) {
    !elm.parent.empty() && flat.add(elm.parent);
  }
