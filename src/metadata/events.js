...
export function event_src({wsql, md, utils, classes, adapters: {pouch}, cat}) {
  ...
  const user_log_in = () => {
    ...
    if(!pouch.remote.log) {
      pouch.remote.log = new classes.PouchDB(pouch.dbpath('log'), {skip_setup: true, owner: pouch, fetch: pouch.fetch});
    }
  };
