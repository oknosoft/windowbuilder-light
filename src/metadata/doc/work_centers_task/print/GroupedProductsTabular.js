...
export default function GroupedProductsTabular({nom, obj}) {
  ...
  obj.cutting.find_rows({nom}, (src) => {
    const {calc_order} = src.production;
    ...
    rows.push({
      len: src.len,
      width: src.width,
      number: parseInt(calc_order.number_doc.substring(4)).pad(4),
      partner: calc_order.partner.name,
      other: other.join(','),
      qty: 1,
    })
  });
  ...
  return <table style={{width: '100%', borderCollapse: 'collapse'}} border={1} cellpadding={4}>
    <thead>
    <tr>
      <th style={{width: '20mm'}}>Длина</th>
      <th style={{width: '20mm'}}>Высота</th>
      <th style={{width: '20mm'}}>Номер</th>
      <th>Контрагент</th>
      <th style={{width: '40mm'}}>Обработки</th>
      <th style={{width: '20mm'}}>Шт</th>
    </tr>
    </thead>
    <tbody>
